...
def compute_PCA(df_w_labels, df, file_name, targets, colors, select_on='cat') :
    ...
    ax.set_title('Explained variance ' + '%.2f' % pca.explained_variance_ratio_[0] + ' and ' +  '%.2f' % pca.explained_variance_ratio_[1], fontsize = 20)
...
def swarmplot(expr, data, fig_name):
    print("\n*** Drawing swarmplot, splitting by group")
    measure_data = expr.copy()
    group_names = column_groups.keys()

    typeofmeasure = "mean_enrichment"  # TODO:  transform into an option !
    ...
    sample_colors=dict([ ('Vehicle_cell_T4h-1', 'royalblue'), ('Vehicle_cell_T4h-2', 'blue'), ('Vehicle_cell_T4h-3', 'dodgerblue'),
                             ('Pellets_cell_T4h-1', 'green'), ('Pellets_cell_T4h-2', 'darkgreen'), ('Pellets_cell_T4h-3', 'mediumseagreen'),
                             ('Myelin_cell_T4h-1', 'orangered'), ('Myelin_cell_T4h-2', 'mediumvioletred'), ('Myelin_cell_T4h-3', 'crimson')])

    fig, ax = plt.subplots(figsize=(12,6))
    melted["metabolite"] = melted["metabolite"].apply(lambda x: x[:8]) #shorten the metabolite names
...
samples = ['Vehicle_cell_T4h-1', 'Vehicle_cell_T4h-2', 'Vehicle_cell_T4h-3', 'Pellets_cell_T4h-1', 'Pellets_cell_T4h-2', 'Pellets_cell_T4h-3', 'Myelin_cell_T4h-1',	'Myelin_cell_T4h-2', 'Myelin_cell_T4h-3']
...
all_colors = ['royalblue', 'blue', 'dodgerblue',
              'green', 'darkgreen', 'mediumseagreen',
              'orangered', 'mediumvioletred', 'crimson']

compute_PCA(reduced.T.reset_index(), values_no_labels, "all_PCA.png", targets = Metabolites.columns.values, colors = all_colors)

swarmplot(reduced.T.reset_index(drop=True), measure_data.reset_index(), "swarmplot_v2.png",)
